...
#####################
# Cash Dataset Related Flags #
#####################
tf.app.flags.DEFINE_bool(
    'regression', False,
    'Whether or not to change softmax to regression in loss.')

tf.app.flags.DEFINE_bool(
    'cashDB', False,
    'Whether or not to use Cash dataset.')
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    #########################################
    # Custom loss function for Cash dataset #
    #########################################
    def custom_loss_for_cashDB(labels, predictions):
      total_loss = 0
      #i = 1, N = dataset.num_classes
      #for i <= N:
      #  total_loss++
    ...
    def clone_fn(batch_queue):
      ...
      #############################
      if FLAGS.cashDB:
        custom_loss_for_cashDB(labels=labels, predictions=logits)
        return end_points
